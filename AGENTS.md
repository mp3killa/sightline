# Sightline for Codex — internal notes

**Sightline** (formerly "Codex Panel") is a native **Android Studio / IntelliJ plugin** that
wraps the `Codex` CLI in a graphical chat panel (message bubbles, collapsible tool cards, diffs,
...
| File | Role |
|---|---|
| `ClaudeToolWindowFactory.java` | Registers the right-dock "Sightline" tool window (`<toolWindow id="Sightline">`). **The one Java file in the plugin, deliberately:** a Kotlin class implementing `ToolWindowFactory` materialises bridges into the interface's deprecated/experimental default methods (`isApplicable`, `isDoNotActivateOnStart`, `getAnchor`, `getIcon`, `manage`), which the Plugin Verifier reports and a future platform removal would break. Java inherits them instead. |
...
| `activity/*.kt` | Platform-free, unit-tested **observable-event layer** feeding the status strip: `AgentActivityEvent`, `ActivityInterpreter`, `OutputParsers` (Gradle/compiler/test + adb/lint/detekt/ktlint console output and Android device signals), `ReportParsers` + `BuildReportScanner` (JUnit/detekt/ktlint/lint XML + SARIF, read off-EDT). The graph model, reducer, classifier, lenses, glyphs, density/label systems and headless renderer were removed with the activity map. |
...
| `vcs/*.kt` | **Generate commit message** (`Vcs.MessageActionGroup` toolbar button in the commit dialog): `GenerateCommitMessageAction` reads the changes being committed, builds their unified diff (`IdeaTextPatchBuilder` → `UnifiedDiffWriter`, in a read action, off-EDT), and fills the message field via `VcsDataKeys.COMMIT_MESSAGE_CONTROL`. It **disables while a draft is in flight** (a per-project `BUSY` key read in `update()`, cleared on the EDT only once the message is set or an error shown) so a second click can't race the first. `CommitMessageGenerator` runs a **one-shot** `Codex -p` (no session/MCP/tools) with a **fast, low-effort model by default** (`commitMessageModel`, default `haiku`) and a timeout. `CommitStyleScanner`/`CommitStyleExtractor` (the latter platform-free, unit-tested) read the project's **own stated commit style** — a "Commit messages" section in CONTRIBUTING/AGENTS.md/AGENTS.md, a `.gitmessage` template, or a commitlint config — and feed it into the prompt so a house style is honoured without the user repeating it. `CommitMessagePrompt` (platform-free, unit-tested) assembles the prompt (with diff truncation past `MAX_DIFF_CHARS`) and scrubs the reply (strips fences/quotes). Failures surface as a `Sightline` balloon, never a silent no-op. |
| `settings/ClaudeSettings*.kt` | Persisted settings + Settings UI (Settings → Tools → **Sightline for Codex**) |

### The observable-event layer

Every tool/stream event the panel sees is normalised into an `AgentActivityEvent` by
`ActivityInterpreter` and pushed into `StatusModel`. That is what the **status strip's live text**, its
**recovered-failure tally** and the per-turn **processing summary** are made of. The `activity/` package
has **no IntelliJ-platform imports**, so it's covered by plain JUnit4 tests in `src/test`.

This layer was built to drive the **Agent Activity Map**, a live force-directed graph of observable
activity, which was **removed in full on 2026-09-16** (see *Standing decisions*). What it fed remains;
the graph, its UI, the Chat/Split/Map switch and the PSI enrichment that supplied structural edges are
gone, along with the plugin's dependency on `com.intellij.java`.
...
`ui/ChatLayoutPreviewTest` writes `build/chat-layout-{narrow,medium,wide}.png` and
`ui/ChatGalleryPreviewTest` the block-type gallery. Read the PNGs directly; this is the only automated
visual channel. See [docs/TESTING.md](docs/TESTING.md).
...
- **`./gradlew verifyPlugin` does not work — run `tools/verify-plugin.sh`.** IPGP 2.6.0 resolves the IDE
  ...
  the same verifier CLI. **Last run (2026-07-29): `io.mp.sightline:0.6.0` vs `AI-261.25134.95`
...
- **Nothing is persisted but settings — with two named exceptions.** The second (2026-09-16, owner,
  explicit) is a **session id per project**, so a conversation can be resumed after the IDE closes.
  One id and a date, nothing else — no messages, prompts, paths or contents — in the project's own
  `PropertiesComponent`; `ui/SessionMemory` is the **single writer**; off until the user accepts a
  consent dialog; and the UI must never imply the transcript comes back, because Sightline stores none
  and cannot redraw one. Turning it off forgets the id.
- **The Agent Activity Map is removed, and is not coming back as a smaller version of itself.**
  (Decided 2026-09-16, by the owner, to reduce the plugin's surface.) Deleted with it: the graph
  model/reducer/classifier/renderer, the Chat/Split/Map switch and the SPLIT layout, seven settings
  keys, and the PSI/UAST enrichment chain (`ProjectStructureEnricher`, `SourceStructureParser`,
  `NavGraphParser`, `AndroidResourceParser`) that existed only to supply its structural edges — which
  is why the plugin no longer depends on `com.intellij.java`. **Kept**: the observable-event layer it
  was built on, which the status strip, the recovered-failure tally and the per-turn summary read.
  *What survives of its design rule*: **nothing here may claim a relationship or a cause it cannot
  evidence** — still the rule behind `BuildFailureClassifier`'s UNKNOWN, `AndroidFacts`' tier ladder,
  `SessionFailure`'s UNKNOWN and `HealthStatus.UNKNOWN`.
...
- `renderMermaid` (default on) → render ` ```mermaid ` flowchart/state diagrams as native Swing diagrams
  ...
  appends a capability line to the system prompt (`--append-system-prompt`) so Codex reaches for a
...
- `activityReduceMotion` (default off) → static status indicators, no pulsing. The key keeps its
  `activity*` name although the activity map is gone: renaming it would silently reset the choice of
  every user who had turned motion off. The map's other keys were deleted.
...
- `model` (default blank = the CLI's own default) + `customModels` — the **model picker** in the
  ...
  **reports** in `system/init` — which is relayed, never derived, because only the CLI knows that
  `sonnet` resolves to `Codex-sonnet-5`. Switching a **running** session goes over the control protocol
...
- `extraArgs` — advanced: extra CLI args appended verbatim to every `Codex` invocation, split by
