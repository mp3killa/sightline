...
object GradleModuleParser {
    ...
    /** Stands in for "there is no next character". Any char that isn't `/` or `*` would do. */
    private const val NO_NEXT_CHAR = ' '
    ...
    fun stripComments(text: String): String {
        ...
        while (i < text.length) {
            val c = text[i]
            // A sentinel that is neither '/' nor '*', so the last character never looks like a
            // comment opener. Named rather than inline: an unprintable literal here once made
            // this file read as *binary* to grep, which silently skipped it during a rename.
            val next = if (i + 1 < text.length) text[i + 1] else NO_NEXT_CHAR
            when {
                inString -> {
                    sb.append(c)
